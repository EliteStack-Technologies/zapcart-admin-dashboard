...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    ...
    if (initialWidth >= 768 && initialWidth <= 1450) {
      return true;
    }
  ...
  useEffect(() => {
    if (windowWidth >= 768 && windowWidth <= 1450) {
      setSidebarCollapsed(true);
    }
  }, [windowWidth >= 768 && windowWidth <= 1450]);

  const isMedRange = windowWidth >= 768 && windowWidth <= 1450;
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <aside className={`
        ...
        ${sidebarCollapsed ? 'w-20' : (isMedRange ? 'w-56' : 'w-64')} h-screen bg-sidebar border-r border-sidebar-border
        transition-all duration-300 ease-in-out
      `}>
        <div className={`flex flex-col h-full ${sidebarCollapsed ? 'p-2' : (isMedRange ? 'p-4' : 'p-6')}`}>
          <div className={`flex items-center justify-between ${sidebarCollapsed ? 'mb-8 flex-col gap-2' : (isMedRange ? 'mb-5' : 'mb-8')}`}>
            <div className={sidebarCollapsed ? 'flex flex-col items-center' : ''}>
              <h1 className={sidebarCollapsed ? "text-xl font-bold text-sidebar-foreground" : (isMedRange ? "text-xl font-bold text-sidebar-foreground" : "text-2xl font-bold text-sidebar-foreground")}>
                {sidebarCollapsed ? "" : "ZapGoCart"}
              </h1>
              {!sidebarCollapsed && (
                <p className={`text-sidebar-foreground/60 mt-1 ${isMedRange ? 'text-xs' : 'text-sm'}`}>Management Dashboard</p>
              )}
            </div>
          ...
          <nav className={`flex-1 overflow-y-auto overflow-x-hidden min-h-0 pr-2 scrollbar-custom scroll-smooth ${isMedRange && !sidebarCollapsed ? 'space-y-0.5' : 'space-y-1'}`}>
            {navItems.map((item) => (
              <div key={item.to}>
                <NavLink
                  to={item.to}
                  className={`flex items-center rounded-lg text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors ${
                    sidebarCollapsed 
                      ? 'gap-3 px-3 py-2.5 justify-center' 
                      : (isMedRange ? 'gap-2 px-2.5 py-2 text-xs' : 'gap-3 px-3 py-2.5 text-sm')
                  }`}
                  activeClassName="bg-sidebar-accent text-sidebar-accent-foreground"
                >
                  <item.icon className={`transition-all duration-300 ${(!sidebarCollapsed && isMedRange) ? 'w-4 h-4' : 'w-5 h-5'}`} />
                  {!sidebarCollapsed && <span className={`font-medium ${isMedRange ? 'text-xs' : 'text-sm'}`}>{item.label}</span>}
                </NavLink>
                ...
                {item.to === '/products' && inventoryEnabled && (
                  <div className="space-y-1 mt-1">
                    <button
                      ...
                      className={`w-full flex items-center rounded-lg text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors ${
                        location.pathname.startsWith('/inventory') ? 'bg-sidebar-accent text-sidebar-accent-foreground' : ''
                      } ${
                        sidebarCollapsed 
                          ? 'gap-3 px-3 py-2.5 justify-center' 
                          : (isMedRange ? 'gap-2 px-2.5 py-2 justify-between' : 'gap-3 px-3 py-2.5 justify-between')
                      }`}
                    >
                      <div className={`flex items-center ${
                        sidebarCollapsed 
                          ? 'gap-3 justify-center' 
                          : (isMedRange ? 'gap-2' : 'gap-3')
                      }`}>
                        <PackageOpen className={`transition-all duration-300 ${(!sidebarCollapsed && isMedRange) ? 'w-4 h-4' : 'w-5 h-5'}`} />
                        {!sidebarCollapsed && <span className={`font-medium ${isMedRange ? 'text-xs' : 'text-sm'}`}>Inventory</span>}
                      </div>
                      {!sidebarCollapsed && (
                        <ChevronDown
                          className={`transition-transform duration-200 ${
                            isMedRange ? 'w-3 h-3' : 'w-4 h-4'
                    ...
                    {inventoryExpanded && !sidebarCollapsed && (
                      <div className={`border-l-2 border-sidebar-border ${
                        isMedRange 
                        ...
                        {inventorySubItems.map((subItem) => (
                          <NavLink
                            key={subItem.to}
                            to={subItem.to}
                            className={`flex items-center rounded-lg text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground transition-colors ${
                              isMedRange 
                          ...
                          >
                            <subItem.icon className={`transition-all duration-300 ${isMedRange ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
          ...
          <div className={`border-t border-sidebar-border ${
            sidebarCollapsed 
              ? 'pt-4 px-0' 
              : (isMedRange ? 'pt-3 px-2' : 'pt-4 px-3')
          }`}>
            <Button
              variant="outline"
              className={`w-full justify-start text-red-600 hover:text-red-700 hover:bg-red-50 ${
                sidebarCollapsed 
                  ? 'px-0 flex justify-center gap-2' 
                  : (isMedRange ? 'px-2 py-1.5 gap-1.5 text-xs' : 'px-3 py-2 gap-2 text-sm')
              }`}
              onClick={handleLogout}
            >
              <LogOut className={`transition-all duration-300 ${isMedRange ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
      ...
      <main className={`flex-1 w-full flex flex-col min-h-screen ${
        sidebarCollapsed 
          ? 'md:ml-20' 
          : (isMedRange ? 'md:ml-56' : 'md:ml-64')
      } transition-all duration-300`}>
        {/* Mobile Header */}
        <div className="md:hidden sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b px-4 py-2 flex items-center justify-between">
        ...
        {/* Desktop Header */}
        <div className={`hidden md:block sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b transition-all duration-300 ${
          isMedRange ? 'px-4 py-3' : 'px-6 py-4'
        }`}>
          <div className="flex items-center justify-between">
            <div>
              <h1 className={`font-bold text-foreground ${isMedRange ? 'text-lg' : 'text-xl'}`}>Admin Dashboard</h1>
              <div className={`flex items-center ${isMedRange ? 'gap-1.5' : 'gap-2'}`}>
                <p className={`text-muted-foreground font-medium ${isMedRange ? 'text-xs' : 'text-sm'}`}>Welcome back, {user?.name || 'Admin'}</p>
                <span className="text-xs text-muted-foreground/40">|</span>
                <p className={`font-bold text-primary uppercase tracking-wide ${isMedRange ? 'text-[10px]' : 'text-xs'}`}>{user?.business_name}</p>
              </div>
            </div>
        ...
        <div className={`flex-1 p-2 transition-all duration-300 ${
          isMedRange 
            ? 'sm:p-4 md:p-5 pb-32 md:pb-5' 
            : 'sm:p-6 md:p-8 pb-32 md:pb-8'
        }`}>
          {children}
        </div>

        {/* Mobile Bottom Navigation - Floating Dock */}
        <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-sm">
