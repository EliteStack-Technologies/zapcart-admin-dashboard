...
import { TrendingUp, Package, Search, Check, ChevronsUpDown } from 'lucide-react';
...
import { addStockIn, getStockInHistory, StockInEntry } from '@/services/inventory';
...
const StockIn = () => {
  ...
  const [stockHistory, setStockHistory] = useState<StockInEntry[]>([]);
  ...
  const fetchStockHistory = async () => {
    try {
      const response = await getStockInHistory();
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-4xl font-bold text-foreground">Stock In</h1>
            <p className="text-sm text-gray-500">Register incoming inventory.</p>
          </div>
        </div>
                ...
                {filteredHistory.length === 0 ? (
                  ...
                  filteredHistory.map((entry, index) => (
                    <TableRow key={entry._id}>
                      ...
                      <TableCell className="text-sm text-blue-600">{entry.reason}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <DialogTitle className="text-xl">Stock In</DialogTitle>
            </div>
          </DialogHeader>
