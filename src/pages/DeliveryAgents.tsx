...
import { deleteDeliveryAgent, getDeliveryAgents, DeliveryAgent } from "@/services/deliveryAgents";
...
const DeliveryAgents = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setAgents(Array.isArray(data) ? data : data?.deliveryAgents || data?.data || []);
    } catch (error: any) {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
              <Truck className="w-8 h-8 text-primary" /> Delivery Agents
            </h1>
            <p className="text-muted-foreground mt-2">
              Manage your fleet of delivery personnel
            </p>
          </div>
